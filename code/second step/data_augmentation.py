...
class RandomSubImagesAroundLandmarks:
    ...
    def __call__(self, smp):
        ...
        sub_images = [image[si[0]:si[0] + size, si[1]:si[1] + size, si[2]:si[2] + size] for si in random_start_idcs]
...
def create_identity_grid3D(shape, spacing, offset_center=False):
...
def create_landmark_grid3D(shape, spacing, landmarks):
    grid = create_identity_grid3D(shape, spacing, offset_center=True)
    grid = landmarks[:, :, None, None, None] - grid[None]
    return grid

class ConvertToTrainingSample(object):
    def __init__(self, downsample_factor, log_fn):
        self.downsample_factor = downsample_factor
        self.logfn = log_fn

    def __call__(self, sample):
        image, landmarks = sample['image'], sample['landmarks']
        landmarks = np.copy(landmarks[:, ::-1])
        shape = np.array(image.shape) // self.downsample_factor
        spacing = np.array(sample['spacing']) * self.downsample_factor
        cls = create_landmark_classmask(shape, spacing, landmarks).astype(np.float32)
        rgr = create_landmark_grid(shape, spacing, landmarks).astype(np.float32)
...
class ConvertToTrainingSample3D(object):
    ...
    def __call__(self, sample):
        ...
        rgr = [create_landmark_grid3D(shape, spacing, landmarks[x, None]).astype(np.float32) for x in nr_ims]
        cls = np.stack(cls)
        rgr = np.stack(rgr)
        cls = np.squeeze(cls, axis=1)
        rgr = np.squeeze(rgr, axis=1)
...
#
#     def __init__(self, dataset='train', root_dir='/home/bob/data/Cephalometric', transform=None, return_avg_gt=False):
#         self.root_dir = Path(root_dir)
#         self.transform = transform
#
#         self.rs = np.random.RandomState(808)
#         patids = np.arange(150, dtype=int) + 1
#         # self.rs.shuffle(patids)
#         if dataset == 'all':
#             pass
#         elif dataset == 'train':
#             patids = patids[:140]
...
#             patids = patids[140:]
#         else:
#             raise Exception('Foutje, bedankt!')
#
#         lm_fn = None
#         if return_avg_gt:
#             lm_fn = lambda junior, senior: np.add(junior, senior) / 2.
#         else:
#             lm_fn = lambda junior, senior: junior if self.rs.rand() < 0.5 else senior
#         self.lm_fn = lm_fn
#
#         self.patids = patids
#
...
#         image, junior, senior = self.load_sample(self.patids[idx])
#         landmarks = self.lm_fn(junior, senior)
#
#         sample = {'image': image, 'landmarks': landmarks, 'spacing': (1, 1)}
#         if self.transform:
...
#
#     @lru_cache()
#     def load_sample(self, patid):
#         image_fname = self.root_dir / 'RawImage' / 'TrainingData' / f'{patid:03d}.bmp'
#         junior_fname = self.root_dir / '400_junior' / f'{patid:03d}.txt'
#         senior_fname = self.root_dir / '400_senior' / f'{patid:03d}.txt'
#
#         image = np.array(imageio.imread(image_fname)[:, :, 0])
#         image = rescale_intensities(image.astype(np.float32), percentiles=(0, 99))
#         junior = np.loadtxt(junior_fname, delimiter=',', max_rows=19)
#         senior = np.loadtxt(senior_fname, delimiter=',', max_rows=19)
#
...
class CCTADataset(Dataset):
    ...
    def load_sample(self, patid, estimates):
        ...
        if self.single:
            estimated_fname = self.lab_dir + r'_EstimatedSL' + str(self.lm_idx) + r'/' + Path(patid).stem[:-4] + ".nii.txt"
        ...
        if self.single:
            ...
            lms = lms[None, :]
        if estimates:
            estimated_lms = np.loadtxt(estimated_fname)
            if self.single:
                estimated_lms = np.copy(estimated_lms[::-1])
                estimated_lms = estimated_lms[None, :]
            else:
                estimated_lms = np.copy(estimated_lms[:, ::-1])
...
class BulbusDataset(Dataset):
    ...
    def load_sample(self, patid, estimates):
        ...
        estimated_fname = self.lab_dir + r'_EstimatedML/' + Path(patid).stem[:-4] + ".nii.txt"
        if self.single:
            estimated_fname = self.lab_dir + r'_EstimatedSL' + str(self.lm_idx) + r'/' + Path(patid).stem[
                                                                                         :-4] + ".nii.txt"
