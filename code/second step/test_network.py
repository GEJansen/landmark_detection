...
def test_network(network, dataloader, nr_ims, n_class, vs):
    ...
    for imidx, test_batch in tqdm(enumerate(dataloader), total=nr_ims):
        ...
        for c in range(n_class):
            ...
            x_error = (pred[2] - lm[2]) * vs[2]
            distance_errors[imidx, c] = math.sqrt(((z_error ** 2) + (y_error ** 2) + (x_error ** 2)))
...
def test_networkRandC(network, dataloader, nr_ims, n_class, vs, logloss):
    ...
    for imidx, test_batch in tqdm(enumerate(dataloader), total=nr_ims):
        ...
        for c in range(n_class):
            ...
            predx = np.average(locs[2].flatten(), weights=pred_cl.flatten())
            pred = [predz, predy, predx]
            z_error = (predz - lm[0]) * vs[0]
            y_error = (predy - lm[1]) * vs[1]
            x_error = (predx - lm[2]) * vs[2]
            distance_errors[imidx, c] = math.sqrt(((z_error ** 2) + (y_error ** 2) + (x_error ** 2)))
            # print("Predicted lm: ", pred, " Reference lm: ", lm)
            # print("Error: ", distance_errors[imidx, c])
...
def main(task, dataset, choosen_iter, heatmap):
    ...
    if "CCTA" in args.data_dir:
        validation_set = data.CCTADataset(dataset, transform=transforms_validation, vs=args.voxel_size, single=args.lm >= 0,
                                          lm_idx=args.lm)
        vs = (args.voxel_size, args.voxel_size, args.voxel_size)
    elif 'Bulbus' in args.data_dir:
        validation_set = data.BulbusDataset(dataset, transform=transforms_validation, chunk_size=patch_size, single=args.lm >= 0, lm_idx=args.lm)
    ...
    if task == "validation":
        ...
        df.to_excel(save_dir + "_TIMES_" + task + "_" + dataset + "_" + str(choosen_iter) + "CONSEC.xlsx", index = False)
