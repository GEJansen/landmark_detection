from networks_3D import Network, OldRCM3D_tensorflowmodel, MultiFineTune
import torch
import numpy as np
import argparse
import os
...
def parse_args():
    ...
    parser.add_argument('--rs', type=int, default=123)
...
def main():
    ...
    if RandC:
        transforms_training = transforms.Compose(
            [data.RandomSubImagesAroundLandmarks(size=patch_size, rs=np.random.RandomState(808)),
             data.ConvertToTrainingSample3D(2, logfn, args.lm)])
    ...
    print('#Load Data')
    if "CCTA" in args.data_dir:
        training_set = data.CCTADataset('train', transform=transforms_training, vs=args.voxel_size, single = args.lm >= 0, lm_idx=args.lm)
        validation_set = data.CCTADataset('val', transform=transforms_validation, vs=args.voxel_size, single = args.lm >= 0, lm_idx=args.lm)
        # validation_set_full_images = data.CCTADataset('val', vs=args.voxel_size, single = False, lm_idx=args.lm)
    elif 'Bulbus' in args.data_dir:
        training_set = data.BulbusDataset('train', transform=transforms_training, chunk_size=patch_size, single = args.lm >= 0, lm_idx=args.lm)
        validation_set = data.BulbusDataset('val', transform=transforms_validation, chunk_size=patch_size, single = args.lm >= 0, lm_idx=args.lm)
        ...
        for it, training_batch in tqdm(enumerate(data_loader_training), total=args.max_iters):
            ...
            if not it % args.evaluate_model_every:
                ...
                with torch.no_grad():
                    ...
                    ims_val = validaton_batch['image'].cuda()
